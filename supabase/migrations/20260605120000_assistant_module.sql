...
-- - Tablas en zentra_erp (no en schema propio) para no requerir cambios en
...
--   (proyecto Supabase original) o en `zentra_erp` (instalación Zentra / instancia
...
-- 1) Corpus: documentos
-- ---------------------------------------------------------------------------
create table if not exists zentra_erp.assistant_kb_documents (
...
comment on table zentra_erp.assistant_kb_documents is
...
-- ---------------------------------------------------------------------------
create table if not exists zentra_erp.assistant_kb_chunks (
  id uuid primary key default gen_random_uuid(),
  document_id uuid not null references zentra_erp.assistant_kb_documents(id) on delete cascade,
...
create index if not exists idx_assistant_kb_chunks_tsv
  on zentra_erp.assistant_kb_chunks using gin (tsv);
create index if not exists idx_assistant_kb_chunks_document
  on zentra_erp.assistant_kb_chunks (document_id);

comment on table zentra_erp.assistant_kb_chunks is
...
-- ---------------------------------------------------------------------------
create table if not exists zentra_erp.assistant_conversations (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists idx_assistant_conversations_empresa
  on zentra_erp.assistant_conversations (empresa_id, created_at);

create table if not exists zentra_erp.assistant_messages (
  id uuid primary key default gen_random_uuid(),
  conversation_id uuid not null references zentra_erp.assistant_conversations(id) on delete cascade,
  empresa_id uuid not null,
  usuario_id uuid,
...
create index if not exists idx_assistant_messages_conversation
  on zentra_erp.assistant_messages (conversation_id, created_at);
create index if not exists idx_assistant_messages_rate_limit
  on zentra_erp.assistant_messages (usuario_id, role, created_at);
...
-- ---------------------------------------------------------------------------
alter table zentra_erp.assistant_kb_documents enable row level security;
alter table zentra_erp.assistant_kb_chunks enable row level security;
alter table zentra_erp.assistant_conversations enable row level security;
alter table zentra_erp.assistant_messages enable row level security;

do $$
begin
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    drop policy if exists assistant_kb_documents_select on zentra_erp.assistant_kb_documents;
    create policy assistant_kb_documents_select
      on zentra_erp.assistant_kb_documents for select
      to authenticated using (true);

    drop policy if exists assistant_kb_chunks_select on zentra_erp.assistant_kb_chunks;
    create policy assistant_kb_chunks_select
      on zentra_erp.assistant_kb_chunks for select
      to authenticated using (true);
  end if;
end
$$;
...
-- ---------------------------------------------------------------------------
create or replace function zentra_erp.assistant_search_kb(
  p_query text,
...
as $$
  ...
  from zentra_erp.assistant_kb_chunks c
  join zentra_erp.assistant_kb_documents d on d.id = c.document_id
  cross join qq
...
comment on function zentra_erp.assistant_search_kb is
...
begin
  if exists (select 1 from pg_roles where rolname = 'service_role') then
    grant usage on schema zentra_erp to service_role;
    grant all on zentra_erp.assistant_kb_documents to service_role;
    grant all on zentra_erp.assistant_kb_chunks to service_role;
    grant all on zentra_erp.assistant_conversations to service_role;
    grant all on zentra_erp.assistant_messages to service_role;
    grant execute on function zentra_erp.assistant_search_kb(text, text[], text, int) to service_role;
  end if;
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    grant usage on schema zentra_erp to authenticated;
    grant select on zentra_erp.assistant_kb_documents to authenticated;
    grant select on zentra_erp.assistant_kb_chunks to authenticated;
    grant execute on function zentra_erp.assistant_search_kb(text, text[], text, int) to authenticated;
  end if;
end
$$;
