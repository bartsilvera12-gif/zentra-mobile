...
import NotaCreditoDetalleClient from "@/app/notas-credito/[id]/NotaCreditoDetalleClient";

export type NotaCreditoDetalleModalProps = {
  id: string | null;
  open: boolean;
  onClose: () => void;
};

export default function NotaCreditoDetalleModal({ id, open, onClose }: NotaCreditoDetalleModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-4 md:p-6">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="nc-detalle-titulo"
        className="relative flex h-[100dvh] max-h-[100dvh] w-full max-w-5xl flex-col overflow-hidden rounded-none border-0 border-slate-200 bg-white shadow-2xl shadow-[#4FAEB2]/10 ring-1 ring-[#4FAEB2]/15 sm:h-[92dvh] sm:max-h-[1000px] sm:rounded-2xl sm:border"
      >
        <span
          aria-hidden="true"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto bg-slate-50/50 px-5 py-5">
          <NotaCreditoDetalleClient id={id} variant="modal" onClose={requestClose} />
        </div>
      </div>
    </div>
  );
