...
import { parseProveedoresRows, buildProvResolverMaps, buildProvPreview, commitProveedores } from "@/lib/imports/proveedores-importer";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const parsed = parseProveedoresRows(res.ctx.rows);
    const maps = await buildProvResolverMaps(res.ctx.schema, res.ctx.empresaId);
    buildProvPreview(parsed, maps);
    const out = await commitProveedores(res.ctx.schema, res.ctx.empresaId, parsed, maps, res.ctx.crearFaltantes);
    const auditId = await registrarImportAudit(res.ctx.schema, res.ctx.empresaId, {
      entidad: "proveedores", filename: res.ctx.filename, total_rows: parsed.length,
    ...
    console.error("[proveedores/import/commit]", e instanceof Error ? e.message : e);
