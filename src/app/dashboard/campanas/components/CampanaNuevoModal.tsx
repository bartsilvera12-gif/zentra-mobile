...
import CampanasNuevoClient from "../nuevo/CampanasNuevoClient";

export type CampanaNuevoModalProps = {
  open: boolean;
  onClose: () => void;
  onCreated?: (id: string) => void;
};

export default function CampanaNuevoModal({
  open,
  onClose,
  onCreated,
}: CampanaNuevoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-4 md:p-6">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="campana-nuevo-titulo"
        className="relative flex h-[100dvh] max-h-[100dvh] w-full max-w-4xl flex-col overflow-hidden rounded-none border-0 border-slate-200 bg-white shadow-2xl shadow-[#4FAEB2]/10 ring-1 ring-[#4FAEB2]/15 sm:h-[94dvh] sm:max-h-[1100px] sm:rounded-2xl sm:border"
      >
        <span
          aria-hidden="true"
