...
import ColaEditor from "./ColaEditor";

export type ColaEditorModalProps = {
  queueId: string | null;
  /** "Nueva cola" si es draft recién creada, "Editar cola" si ya existía. */
  isNew?: boolean;
  open: boolean;
  onClose: () => void;
  onSaved: () => void;
...
export default function ColaEditorModal({
  queueId,
  isNew = false,
  open,
  onClose,
      ...
      >
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 bg-gradient-to-br from-white via-white to-[#4FAEB2]/5 px-6 pb-5 pt-6">
          <div className="min-w-0 flex-1">
            ...
            <h2
              id="cola-editor-titulo"
              className="mt-1 truncate text-2xl font-semibold tracking-tight text-slate-900"
            >
              {isNew ? "Nueva cola" : "Editar cola"}
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              {isNew
                ? "Definí nombre, canales asociados, estrategia de distribución y agentes."
                : "Modificá nombre, canales, estrategia y agentes de esta cola."}
            </p>
          </div>
          <button
            type="button"
